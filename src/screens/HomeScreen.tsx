  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <View style={styles.info}>
        <Text style={styles.name} numberOfLines={1}>{item.tensp}</Text>
        <Text style={styles.category}>🏷️ {item.loaisp}</Text>
        <Text style={styles.price}>{formatPrice(item.gia)}</Text>
        <Text style={styles.idText} numberOfLines={1}>ID: {item.idsanpham}</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  noImg: { backgroundColor: '#ecf0f1', justifyContent: 'center', alignItems: 'center' },
  ...
  price: { fontSize: 15, fontWeight: 'bold', color: '#e74c3c', marginBottom: 2 },
